...
export default function WhatsAppButton() {
  ...
  if (!config?.telefone) return null

  // Formatar número para WhatsApp (remover caracteres especiais)
  const phoneNumber = config.telefone.replace(/\D/g, "")

  // Mensagem pré-definida
  const message = encodeURIComponent(
    `Olá! Vi o site de ${config.nome_empresa} e gostaria de saber mais sobre os móveis artesanais.`,
  )
  ...
  return (
    ...
    >
      <svg className="h-7 w-7 md:h-8 md:w-8" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
